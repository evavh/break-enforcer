		...
		systemd.services.break-enforcer = {
			...
			serviceConfig = {
				...
				ExecStart = ''
					...
					${concatMapStrings (x: "--break-start-notify " + x)
					cfg.break-start-notify} \
					${concatMapStrings (x: "--break-end-notify " + x)
					cfg.break-end-notify} \
					${concatMapStrings (x: "--work-reset-notify " + x)
					cfg.work-reset-notify} \
